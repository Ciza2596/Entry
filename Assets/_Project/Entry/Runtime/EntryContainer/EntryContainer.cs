...
namespace CizaEntry
{
	public class EntryContainer
	{
		...
		public void Bind<TKey, TInstance>(TInstance instance) where TKey : class where TInstance : class =>
			Bind(typeof(TKey), instance);

		public void BindAndSelf<TKey, TInstance>(TInstance instance) where TKey : class where TInstance : class
		{
			...
			where TKey : class
		{
			...
			var key = typeof(TKey);
			Assert.IsTrue(!_entryPoints.Contains(key), $"[EntryContainer::TryResolve] Can't use key: {key} to find instance.");
		...
		private void Bind<TInstance>(Type key, TInstance instance)
		{
			...
			Assert.IsTrue(CheckCanBeKey(key, instanceType), $"[EntryContainer::Bind] Please check instanceType: {instanceType} inherits key: {key}.");

			if (_instanceTypeMap.TryGetValue(key, out var currentInstanceType))
			{
				Debug.LogWarning($"[EntryContainer::Bind] Key: {key} is already registered. Current matching instanceType: {currentInstanceType}");
				return;
			}
		...
		private void RemoveInstanceByInstanceType(Type instanceType)
		{
			if (!_instanceDataMap.TryGetValue(instanceType, out var instanceData))
			{
				Debug.LogWarning($"[EntryContainer::RemoveInstanceByInstanceType] InstanceType: {instanceType} is already removed.");
				return;
			}
		...
		private void RemoveInstanceByKey(Type key)
		{
			...
			Assert.IsTrue(hasValue, $"[EntryContainer::RemoveInstanceByKey] Key: {key} is already removed.");
